...
const DEFAULT_ITEMS=[{name:'Aata',qty:5,minQty:2,emoji:'🌾'},{name:'Doodh',qty:2,minQty:1,emoji:'🥛'},{name:'Chawal',qty:3,minQty:2,emoji:'🍚'},{name:'Dal',qty:2,minQty:1,emoji:'🫘'},{name:'Aloo',qty:8,minQty:3,emoji:'🥔'},{name:'Pyaz',qty:6,minQty:3,emoji:'🧅'},{name:'Tamatar',qty:4,minQty:2,emoji:'🍅'},{name:'Ghee',qty:1,minQty:1,emoji:'🧈'},{name:'Namak',qty:1,minQty:1,emoji:'🧂'},{name:'Chai',qty:1,minQty:1,emoji:'🍵'}];
...
function loadState(){const si=localStorage.getItem(profileKey('inventory'));inventory=si?JSON.parse(si):DEFAULT_ITEMS.map((item,i)=>({id:Date.now()+i,name:item.name,qty:item.qty,minQty:item.minQty||1,emoji:item.emoji,addedOn:Date.now()}));shoppingList=JSON.parse(localStorage.getItem(profileKey('shopping'))||'[]');expenses=JSON.parse(localStorage.getItem(profileKey('expenses'))||'[]');expiryItems=JSON.parse(localStorage.getItem(profileKey('expiry'))||'[]');}
...
let toastTimer;
function showToast(msg,type=''){let toast=document.getElementById('toast');if(!toast){toast=document.createElement('div');toast.id='toast';document.body.appendChild(toast);}toast.textContent=msg;toast.className='toast show'+(type?' toast-'+type:'');clearTimeout(toastTimer);toastTimer=setTimeout(()=>toast.className='toast',2500);}
...
function renderInventory(){
  ...
  items.forEach((item,idx)=>{
    ...
    if(editingId===item.id){
      card.innerHTML=`<span class="item-emoji">${item.emoji}</span><div class="edit-form"><input class="edit-name-input" value="${escapeHtml(item.name)}" id="editName${item.id}"/><div class="edit-row"><label>${t('qtyLabel')}</label><input type="number" class="edit-qty-input" value="${item.qty}" min="0" id="editQty${item.id}"/><label>${t('minQtyLabel')}</label><input type="number" class="edit-min-input" value="${item.minQty||1}" min="1" id="editMin${item.id}"/></div><div class="edit-actions"><button class="btn-save-edit" onclick="saveEdit(${item.id})">${t('saveEdit')}</button><button class="btn-cancel-edit" onclick="cancelEdit()">${t('cancel')}</button></div></div>`;
    }else{
      card.innerHTML=`<span class="item-emoji">${item.emoji}</span><div class="item-info"><div class="item-name">${escapeHtml(item.name)}</div><div class="item-status status-${sc}">${sl}</div></div><div class="item-controls"><button class="btn-qty minus" data-id="${item.id}">−</button><span class="qty-display ${isOut?'zero':isLow?'low':''}">${item.qty}</span><button class="btn-qty plus" data-id="${item.id}">+</button></div><div class="item-actions"><button class="btn-edit" data-id="${item.id}">✏️</button><button class="btn-delete" data-id="${item.id}">✕</button></div>`;
    }
...
function renderShoppingList(){
  ...
  shoppingList.forEach(item=>{
    ...
    card.innerHTML=`<div class="shopping-pulse"></div><span class="item-emoji">${item.emoji}</span><div class="shopping-info"><div class="shopping-name">${escapeHtml(item.name)}</div><div class="shopping-sub">${t('needToBuy')}</div></div><button class="btn-restock" data-id="${item.id}">${t('restock')}</button>`;
    list.appendChild(card);
  });
}
...
function renderBudget(){
  ...
  if(el('budgetLeftNum')){el('budgetLeftNum').textContent=(left<0?'-':'')+'₹'+Math.abs(left).toLocaleString('en-IN');el('budgetLeftNum').style.color=left<0?'var(--c-red)':'var(--c-green)';}
  if(el('budgetSetInput'))el('budgetSetInput').value=budget||'';
  if(el('budgetProgress')&&budget>0){const pct=Math.min(100,(totalSpent/budget)*100);el('budgetProgress').style.width=pct+'%';el('budgetProgress').style.background=pct>90?'var(--c-red)':pct>70?'var(--c-orange)':'var(--c-green)';}
...
function addItemToInventory(name,qty,minQty){
  ...
  qty=parseInt(qty)||1;minQty=parseInt(minQty)||1;
  const ex=inventory.find(i=>i.name.toLowerCase()===name.toLowerCase());
  if(ex){ex.qty+=qty;shoppingList=shoppingList.filter(s=>s.id!==ex.id);}
  else inventory.unshift({id:Date.now(),name,qty,minQty,emoji:getEmoji(name),addedOn:Date.now()});
  renderAll();showToast(t('toastAdded'),'success');
}
function decreaseQty(id){const item=inventory.find(i=>i.id===id);if(!item||item.qty<=0)return;item.qty--;if(item.qty===0&&!shoppingList.find(s=>s.id===id)){shoppingList.push({id:item.id,name:item.name,emoji:item.emoji});showToast(t('toastAutoCart'),'cart');}renderAll();}
function increaseQty(id){const item=inventory.find(i=>i.id===id);if(!item)return;item.qty++;shoppingList=shoppingList.filter(s=>s.id!==id);renderAll();}
...
function restockItem(id){const inv=inventory.find(i=>i.id===id);if(inv)inv.qty=Math.max(inv.minQty||1,5);shoppingList=shoppingList.filter(s=>s.id!==id);renderAll();showToast(t('toastRestocked'),'success');}
...
function saveEdit(id){const item=inventory.find(i=>i.id===id);if(!item)return;const n=document.getElementById('editName'+id);const q=document.getElementById('editQty'+id);const m=document.getElementById('editMin'+id);if(n)item.name=n.value.trim()||item.name;if(q)item.qty=Math.max(0,parseInt(q.value)||0);if(m)item.minQty=Math.max(1,parseInt(m.value)||1);item.emoji=getEmoji(item.name);if(item.qty===0&&!shoppingList.find(s=>s.id===id))shoppingList.push({id:item.id,name:item.name,emoji:item.emoji});else if(item.qty>0)shoppingList=shoppingList.filter(s=>s.id!==id);editingId=null;renderAll();showToast(t('saveEdit')+' ✓','success');}
function cancelEdit(){editingId=null;renderInventory();}
function shareOnWhatsApp(){if(shoppingList.length===0)return;const h=`🛒 *${t('shoppingList')} — Smart Serve*\n_${currentProfile?.name||''}_\n\n`;const items=shoppingList.map((item,i)=>`${i+1}. ${item.emoji} ${item.name}`).join('\n');window.open('https://wa.me/?text='+encodeURIComponent(h+items),'_blank');}
...
function openLangModal(){const modal=document.getElementById('langModal');const grid=document.getElementById('langGrid');grid.innerHTML='';LANGUAGES.forEach(lang=>{const btn=document.createElement('button');btn.className='lang-option'+(lang.code===currentLang?' active':'');btn.innerHTML=`<strong>${lang.nativeLabel}</strong><small>${lang.label}</small>`;btn.onclick=()=>{setLanguage(lang.code);closeLangModal();renderAll();};grid.appendChild(btn);});modal.classList.remove('hidden');}
function closeLangModal(){document.getElementById('langModal').classList.add('hidden');}

// VOICE
function startVoice(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){showToast('Voice only works in Chrome');return;}const lang=LANGUAGES.find(l=>l.code===currentLang);const rec=new SR();rec.lang=lang?lang.voice:'hi-IN';rec.interimResults=false;const btn=document.getElementById('voiceBtn');btn.classList.add('listening');btn.textContent='🔴';rec.start();rec.onresult=e=>{const s=e.results[0][0].transcript;document.getElementById('newItemInput').value=s;btn.classList.remove('listening');btn.textContent='🎤';addItemToInventory(s,1,1);};rec.onerror=rec.onend=()=>{btn.classList.remove('listening');btn.textContent='🎤';};}

// SCAN
let scanStream=null,scanInterval=null;
function openScan(){pendingScanName=null;document.getElementById('scanModal').classList.remove('hidden');const btn=document.getElementById('scanAddBtn');btn.disabled=true;btn.style.opacity='0.5';document.getElementById('scanResult').textContent='';document.getElementById('scanStatus').textContent=t('scanDetecting');document.getElementById('scanManualInput').value='';startCamera();}
function closeScan(){stopCamera();pendingScanName=null;document.getElementById('scanModal').classList.add('hidden');document.getElementById('ocrStatus').textContent='';document.getElementById('scanResult').textContent='';const btn=document.getElementById('scanAddBtn');btn.disabled=true;btn.style.opacity='0.5';}
async function startCamera(){const video=document.getElementById('scanVideo');try{scanStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});video.srcObject=scanStream;startFrameAnalysis(video);}catch(e){document.getElementById('scanStatus').textContent='❌ Camera nahi mila — neeche naam likhein';}}
function stopCamera(){if(scanStream){scanStream.getTracks().forEach(t=>t.stop());scanStream=null;}clearInterval(scanInterval);}
function startFrameAnalysis(video){const canvas=document.createElement('canvas');const ctx=canvas.getContext('2d');let detected=false;scanInterval=setInterval(()=>{if(detected||!video.videoWidth)return;canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.drawImage(video,0,0);if('BarcodeDetector' in window){new BarcodeDetector({formats:['ean_13','ean_8','qr_code','code_128','upc_a']}).detect(canvas).then(barcodes=>{if(barcodes.length>0&&!detected){detected=true;lookupBarcode(barcodes[0].rawValue);}}).catch(()=>{});}},800);}
async function lookupBarcode(code){document.getElementById('scanStatus').textContent='🔍 Looking up...';try{const res=await fetch(`https://world.openfoodfacts.org/api/v0/product/${code}.json`);const data=await res.json();if(data.status===1&&data.product){setScanResult(data.product.product_name_hi||data.product.product_name||code,'✅ Product mila!');return;}}catch(e){}setScanResult('Item #'+code,'📦 Unknown — naam edit karein');}
function setScanResult(name,statusMsg){pendingScanName=name;document.getElementById('scanResult').textContent=t('scanFound')+name;document.getElementById('scanStatus').textContent=statusMsg;document.getElementById('scanManualInput').value=name;const btn=document.getElementById('scanAddBtn');btn.disabled=false;btn.style.opacity='1';}
function addScannedItem(){const manual=document.getElementById('scanManualInput').value.trim();const name=manual||pendingScanName;if(name){addItemToInventory(name,1,1);closeScan();}}
...
async function handleOCRImage(e){
  ...
  status.textContent='🔍 Photo padh raha hoon...';ocrBtn.disabled=true;
  try{
    ...
    const response=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:100,messages:[{role:'user',content:[{type:'image',source:{type:'base64',media_type:file.type||'image/jpeg',data:base64}},{type:'text',text:'Is image mein kaunsa grocery/household product hai? Sirf naam likho 1-3 shabd mein. Jaise: Aata, Maggi, Doodh. Koi explanation nahi.'}]}]})});
    const data=await response.json();
    if(data.content?.[0]?.text){const name=data.content[0].text.trim().replace(/["""*]/g,'').split('\n')[0];setScanResult(name,'✅ Photo se naam mila!');status.textContent='✅ Confirm karo ya naam badlo!';}
    else status.textContent='❌ Nahi mila — manually likhein';
  }catch(err){status.textContent='⚠️ Error — naam manually likhein';}
  ocrBtn.disabled=false;e.target.value='';
}
...
function installApp(){if(!deferredInstall)return;deferredInstall.prompt();deferredInstall.userChoice.then(()=>{deferredInstall=null;document.getElementById('installBanner').style.display='none';});}
...
function initEvents(){
  ...
  document.getElementById('addItemBtn').addEventListener('click',()=>{const input=document.getElementById('newItemInput');const qty=document.getElementById('newItemQty');const min=document.getElementById('newItemMin');if(input.value.trim()){addItemToInventory(input.value,qty.value,min.value);input.value='';qty.value=1;min.value=1;input.focus();}});
  ...
  document.getElementById('inventoryList').addEventListener('click',e=>{const id=parseInt(e.target.dataset.id||e.target.closest('[data-id]')?.dataset.id);if(!id)return;if(e.target.classList.contains('minus'))decreaseQty(id);else if(e.target.classList.contains('plus'))increaseQty(id);else if(e.target.classList.contains('btn-delete'))deleteItem(id);else if(e.target.classList.contains('btn-edit'))startEdit(id);});
  document.getElementById('shoppingList').addEventListener('click',e=>{const id=parseInt(e.target.dataset.id);if(id&&e.target.classList.contains('btn-restock'))restockItem(id);});
  ...
  document.getElementById('scanManualInput').addEventListener('input',function(){if(this.value.trim()){pendingScanName=this.value.trim();const btn=document.getElementById('scanAddBtn');btn.disabled=false;btn.style.opacity='1';}});
